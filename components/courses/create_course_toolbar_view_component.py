...
class CreateCourseToolbarViewComponent(BaseComponent):
    ...
    @allure.step('Check visible create course toolbar items')
    def check_visible(self, is_create_course_button_disabled: bool = True):
        self.title.check_visible()
        self.title.check_have_text('Create course')

        if is_create_course_button_disabled:
            self.create_course_button.check_disabled()
        if not is_create_course_button_disabled:
